...
def _build_report(results: list, state_path: Optional[str]) -> str:
    """Bundle a self-contained bug-report markdown blob.

    Includes: aether version, python version, platform, doctor results,
    and tails of the diagnostic log files. Deliberately excludes state
    file contents and backup file contents so private memories aren't
    leaked when the user pastes the report into a GitHub issue. The
    user should still skim the output for paths/usernames they don't
    want public before submitting.
    """
    import platform
    import sys
    try:
    ...
    # Diagnostic log tails. These log message-lengths and event metadata,
    # not message contents — safe to share. The user can verify before
    # pasting.
    for log_name, label in (
        ("encoder_warmup.log", "Encoder warmup log (last 10 lines)"),
        ("auto_ingest.log", "Auto-ingest log (last 10 lines)"),
        ("user_prompt_inject.log", "Prompt-inject log (last 10 lines)"),
    ):
        tail = _read_log_tail(aether_dir / log_name, lines=10)
        if tail is None:
            continue
    ...
    lines.append(
        "_Generated by `aether doctor --report`. Review for paths or "
        "usernames you don't want public before submitting._"
    )
    return "\n".join(lines)
...
def cmd_doctor(args) -> int:
    ...
    if getattr(args, "report", False):
        # Markdown bundle for GitHub issue submissions. Exit code
        # mirrors the underlying check status so CI scripts can chain.
        print(_build_report(results, state_path))
